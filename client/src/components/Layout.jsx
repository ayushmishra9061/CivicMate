    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 dark:bg-slate-950 dark:text-slate-100">
      <aside className="fixed inset-y-0 left-0 z-20 hidden w-64 border-r border-slate-200 bg-white px-4 py-5 dark:border-slate-800 dark:bg-slate-900 lg:block">
        <button onClick={() => navigate('/dashboard')} className="mb-8 flex items-center gap-3 text-left">
          <span className="grid size-11 place-items-center rounded-md bg-civic-blue text-white"><Building2 size={22} /></span>
          <span>
            <span className="block text-lg font-bold">CivicMate</span>
            <span className="text-xs text-slate-500">Smart Citizen Platform</span>
          </span>
        </button>
          ...
          <div className="flex items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-2">
              <div className="relative">
                <Button variant="secondary" className="size-10 px-0" aria-label="Notifications"><Bell size={18} /></Button>
                {unread ? <span className="absolute -right-1 -top-1 rounded-full bg-civic-red px-1.5 text-xs text-white">{unread}</span> : null}
              </div>
              <Button variant="secondary" onClick={() => setDark((value) => !value)} aria-label="Toggle dark mode"><Moon size={18} /></Button>
              <Button variant="secondary" onClick={() => navigate('/profile')} aria-label="Profile"><User size={18} /></Button>
              <Button variant="ghost" onClick={logout}><LogOut size={18} /> Logout</Button>
            </div>
          </div>
          <div className="mt-3 flex gap-2 overflow-x-auto lg:hidden">
            {items.map(({ to, label }) => (
              <NavLink key={to} to={to} className="whitespace-nowrap rounded-md bg-slate-100 px-3 py-2 text-xs font-semibold dark:bg-slate-800">
                {label}
              </NavLink>
            ))}
          </div>
